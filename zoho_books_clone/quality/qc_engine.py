...
def auto_create_qc_for_purchase_receipt(doc, method=None):
    ...
    for row in (getattr(doc, "items", []) or []):
        ...
        try:
            qci_name = create_qc_inspection_for_item(
                doc.doctype, doc.name, item_code, "Incoming"
            )
    ...
    if created:
        frappe.msgprint(
            _("QC Inspection(s) auto-created for {0} item(s): {1}. "
              "Please complete readings before stock is accepted.").format(
                len(created), ", ".join(created)
            ),
            indicator="blue",
            title=_("QC Inspections Created"),
            alert=True,
        )
...
def auto_create_qc_for_stock_entry(doc, method=None):
    ...
    for row in (getattr(doc, "items", []) or []):
        ...
        if not getattr(row, "t_warehouse", None):
            continue
        if not frappe.db.get_value("Item", item_code, "inspection_required_before_manufacture"):
            continue
        ...
        try:
            qci_name = create_qc_inspection_for_item(
                doc.doctype, doc.name, item_code, "In Process"
            )
    ...
    if created:
        frappe.msgprint(
            _("Finished Goods QC Inspection(s) auto-created: {0}. "
              "Please complete readings for the manufactured batch.").format(", ".join(created)),
            indicator="blue",
            title=_("Finished Goods QC Created"),
            alert=True,
        )
...
def _resolve_template(item_code: str, inspection_type: str) -> str | None:
    ...
    # Level 2: item-code specific template
    try:
    ...
    # Level 3: dosage_form match (Ayurvedic)
    dosage_form = None
    try:
    ...
    try:
        tmpl = frappe.db.get_value(
            "QC Inspection Template",
            {"item": ["in", ["", None]], "inspection_type": ["in", [inspection_type, "All"]]},
            "name",
        )
        return tmpl
    except Exception:
