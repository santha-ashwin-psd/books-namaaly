...
<div class="list-page">
  ...
  <div class="sales-toolbar">
    <div class="sales-search">
      ...
      <input v-model="search" placeholder="Search Routings..." class="sales-search-input"/>
    </div>
    ...
    <div class="sales-actions">
      ...
      <button class="sales-btn-primary" @click="openAdd"><span v-html="icon('plus',13)"></span> Add Routing</button>
    </div>
  </div>
  ...
  <div class="inv-table-wrap">
    <table class="inv-table items-desktop-tbl">
      <thead><tr>
        ...
        <th class="sortable" @click="sortBy('name')">Name <span v-html="sortArrow('name')"></span></th>
        <th class="sortable" @click="sortBy('op_count')">Operations <span v-html="sortArrow('op_count')"></span></th>
        <th>Status</th>
        <th class="ta-r sortable" @click="sortBy('modified')">Last Modified <span v-html="sortArrow('modified')"></span></th>
        <th style="width:90px;text-align:center">Actions</th>
      </tr></thead>
      <tbody>
        <template v-if="loading"><tr v-for="n in 5" :key="n"><td colspan="6"><div class="shimmer"></div></td></tr></template>
        <tr v-else-if="!sorted.length"><td colspan="6" class="bk-empty-state"><div class="bk-empty-inner">
          <template v-if="search || filterTab!=='all'">
            ...
            <p class="bk-empty-title">No routings match your filters</p>
          </template>
          <template v-else>
            <p class="bk-empty-title">No Routings yet</p>
            <p class="bk-empty-sub">Create a Routing to define a reusable sequence of operations for your BOMs.</p>
            <button class="bk-empty-btn" @click="openAdd"><span v-html="icon('plus',13)"></span> Add Routing</button>
          </template>
        </div></td></tr>
        <tr v-else v-for="row in paged" :key="row.name" class="inv-row" :class="{selected:selected.has(row.name)}">
          ...
          <td @click="openView(row)" data-label="Name"><span class="inv-link fw-600">{{ row.name }}</span></td>
          <td @click="openView(row)" data-label="Operations"><span class="text-muted">{{ row.op_count != null ? row.op_count : '—' }}</span></td>
          <td @click="openView(row)" data-label="Status">
            <span class="inv-status-badge" :class="row.is_active ? 'status-active' : 'status-inactive'">
...
function openAdd()       { router.push("/manufacturing/routing/new"); }
function openView(row)   { router.push(`/manufacturing/routing/${row.name}`); }
...
async function bulkDelete() {
  ...
  if (!(await confirm({
    title: "Delete selected Routings?",
    body: `Delete ${rows.length} Routing${rows.length > 1 ? "s" : ""}?`,
  ...
  for (const row of rows) {
    if (!(await isRoutingDeletable(row))) { skipped++; continue; }
    try { await apiDelete("Routing", row.name); deleted++; } catch { skipped++; }
  }
  bulkBusy.value = false;
  if (deleted) toast(`Deleted ${deleted} Routing${deleted > 1 ? "s" : ""}` + (skipped ? `, ${skipped} skipped` : ""));
  else toast("No Routings were deleted", "error");
  selected.value.clear();
  load();
}
