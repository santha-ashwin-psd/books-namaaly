...
<div class="list-page">
  ...
  <div class="sales-toolbar">
    <div class="sales-search">
      ...
      <input v-model="search" placeholder="Search Workstation Types..." class="sales-search-input"/>
    </div>
    ...
    <div class="sales-actions">
      ...
      <button class="sales-btn-primary" @click="openAdd"><span v-html="icon('plus',13)"></span> New Workstation Type</button>
    </div>
  </div>
  ...
  <div class="inv-table-wrap">
    <table class="inv-table items-desktop-tbl">
      <thead><tr>
        ...
        <th class="sortable" @click="sortBy('name')">Workstation Type Name <span v-html="sortArrow('name')"></span></th>
        <th>Description</th>
        <th>Status</th>
      ...
      <tbody>
        ...
        <tr v-else-if="!sorted.length"><td colspan="6" class="bk-empty-state"><div class="bk-empty-inner">
          <template v-if="search||filterTab!=='all'">
            ...
            <p class="bk-empty-title">No types match your filters</p>
          </template>
          <template v-else>
            <p class="bk-empty-title">No Workstation Types yet</p>
            <p class="bk-empty-sub">Create your first Workstation Type (e.g., Assembly Line, Testing Station).</p>
            <button class="bk-empty-btn" @click="openAdd"><span v-html="icon('plus',13)"></span> New Workstation Type</button>
          </template>
        </div></td></tr>
        <tr v-else v-for="row in paged" :key="row.name" class="inv-row" :class="{selected:selected.has(row.name)}">
          ...
          <td @click="openView(row)" data-label="Type Name"><span class="inv-link fw-600">{{row.name}}</span></td>
          <td @click="openView(row)" data-label="Description"><span class="text-muted">{{ row.description ? row.description.substring(0,50) + (row.description.length>50?'...':'') : '—' }}</span></td>
...
async function load() {
  loading.value = true;
  try {
    const fields = ["name", "description", "is_active", "modified"];
...
function openView(row) {
  router.push(`/manufacturing/workstation-type/${row.name}`);
}
...
async function bulkDelete() {
  ...
  if (!(await confirm({
    title: "Delete selected Workstation Types?",
    body: `Are you sure you want to delete ${rows.length} Workstation Type${rows.length > 1 ? "s" : ""}?`,
    okLabel: "Delete",
    okStyle: "danger",
  }))) return;
  ...
  for (const row of rows) {
    if (!(await isTypeDeletable(row))) { skipped++; continue; }
    try {
      await apiDelete("Workstation Type", row.name);
      deleted++;
    } catch (e) {
  ...
  if (deleted) toast(`Deleted ${deleted} Workstation Type${deleted > 1 ? "s" : ""}` + (skipped ? `, ${skipped} skipped` : ""));
  else toast("No Workstation Types were deleted", "error");
