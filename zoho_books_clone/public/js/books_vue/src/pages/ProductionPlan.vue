...
<div class="list-page">
  ...
  <div class="sales-toolbar">
    <div class="sales-search">
      ...
      <input v-model="search" placeholder="Search Production Plans…" class="sales-search-input"/>
    </div>
    ...
    <div class="sales-actions">
      ...
      <button class="sales-btn-primary" @click="openAdd"><span v-html="icon('plus',13)"></span> New Production Plan</button>
    </div>
  </div>
  ...
  <div class="inv-table-wrap">
    <table class="inv-table items-desktop-tbl">
      <thead><tr>
        <th class="sortable" @click="sortBy('name')">Production Plan <span v-html="sortArrow('name')"></span></th>
        <th class="sortable" @click="sortBy('posting_date')">Posting Date <span v-html="sortArrow('posting_date')"></span></th>
        <th>Company</th>
        <th>Status</th>
        <th class="ta-r sortable" @click="sortBy('modified')">Last Modified <span v-html="sortArrow('modified')"></span></th>
      </tr></thead>
      <tbody>
        <template v-if="loading"><tr v-for="n in 6" :key="n"><td colspan="5"><div class="shimmer"></div></td></tr></template>
        <tr v-else-if="!sorted.length"><td colspan="5" class="bk-empty-state"><div class="bk-empty-inner">
          <template v-if="search||filterTab!=='all'">
            ...
            <p class="bk-empty-title">No Production Plans match your filters</p>
          </template>
          <template v-else>
            <p class="bk-empty-title">No Production Plans yet</p>
            <p class="bk-empty-sub">Pull demand from open Sales Orders (or add items manually) to plan a production run.</p>
            <button class="bk-empty-btn" @click="openAdd"><span v-html="icon('plus',13)"></span> New Production Plan</button>
          </template>
        </div></td></tr>
        <tr v-else v-for="row in paged" :key="row.name" class="inv-row" @click="openView(row)">
          <td data-label="Production Plan"><span class="inv-link">{{row.name}}</span></td>
          <td data-label="Posting Date" class="text-muted mono-sm">{{fmtDate(row.posting_date)}}</td>
          <td data-label="Company" class="text-muted">{{ row.company }}</td>
...
const tabs = [
  ...
  { key: "Work Orders Created",   label: "Work Orders Created" },
...
function statusClass(row) {
  if (row.status === "Completed" || row.status === "Work Orders Created") return "status-active";
...
function openView(row) {
  router.push(`/manufacturing/production-plan/${row.name}`);
}
